...
\begin{definition}
  We define $\Comp$ to be the finite $\sF$-limit theory generated by sketch consisting of diagrams
  \begin{mathpar}
...
\begin{proof}
  ...
  $u\typ\alpha = \dot u \alpha$ is identity, so since $u$ is a discrete fibration, $\typ\alpha$ is identity as well. Thus we
  ...
  also a 2-cell $\typ\var \to 1_\cU$, which will be the counit of the adjunction. The unit $\eta : 1_{\dot\cU} \to \var\typ$
  is a map of terms, and so is induced by a map $\typ\eta : \typ \to \typ\var\typ = \delta \typ$ such that following square
  commutes:
  \[\begin{codi}
    \obj {|(tl)| u\typ & |(tr)| \set{\typ} \\ |(bl)| \set{\typ} & |(br)| \set{\delta\typ} \\};
    \mor tl \tau:-> tr {{\set{\typ\eta}}}:-> br;
    \mor[swap] tl u\typ\eta:-> bl \tau\var\typ:-> br;
  \end{codi}\]
  We choose $\typ\eta$ to be the unique map $\typ \to \delta\typ$ induced by $\epsilon$ being cartesian and $u\epsilon\typ
  \circ \tau = \pi\typ\circ \tau = 1_{u\typ} = u1_\typ$. Note that then $u\typ\eta = \tau$ and $\epsilon\typ \circ \typ\eta =
  1_\typ$. We show that the above square commutes by showing that both composites are the same unique map induced by a pullback,
  and so they must be equal. In particular, our choice of $\tau\var$ gives us that
  \[\set{\epsilon\typ} \circ \tau\var\typ \circ u\typ\eta = u\typ\eta = \tau = \set{\epsilon\typ \circ \typ\eta} \circ \tau
  = \set{\epsilon\typ} \circ \set{\typ\eta} \circ \tau\]
  and
  \[\pi\delta\typ \circ \tau\var\typ \circ u\typ\eta = \tau = u\typ\eta \circ \pi\typ \circ \tau =
  \pi\delta\typ \circ \set{\typ\eta} \circ \tau, \]
  so $\tau\var\typ \circ u\typ\eta = \set{\typ\eta} \circ \tau$ since the lower right square in the following is a pullback
  \[\begin{codi}[tetragonal=base 7em height 4.5em]
    \obj {|(tl)| u\typ \\ & |(tm)| \set{\delta\typ} & |(tr)| \set{\typ} \\ & |(bm)| \set{\typ} & |(br)| u\typ \\};
    \mor tm {{\set{\epsilon\typ}}}:-> tr \pi\typ:-> br;
    \mor[swap] tm \pi\delta\typ:-> bm \pi\typ:-> br;
    \mor:[bend left] bm {{\tau}}:<- tl {{\tau}}:-> tr;
    \mor[near end]:[dashed, shove=-0.2em] tl {{\set{\typ\eta} \circ \tau}}:-> tm;
    \mor[near end,swap]:[dashed, shove=0.2em] tl {{\tau\var\typ \circ u\typ\eta}}:-> tm;
  \end{codi}\]
  Thus $\typ\eta$ induces a 2-cell $\eta : 1_{\dot \cU} \to \var\typ$. Furthermore, we see that $\epsilon$ is the counit and
  $\eta$ is the unit of an adjunction $\typ \dashv \var$, since the triangle equations $\epsilon\typ \circ \typ\eta = 1_\typ$
  by the construction of $\typ\eta$ and $\var\epsilon \circ \eta\var = 1_\var$ holds if and only if ... \todo{this final bit}
  ...
  \todo{This proof is kinda badly written, but it should be correct.}
\end{proof}
\begin{remark}
...
\begin{proof}
  ...
  \todo{somehow? I can't seem to figure out exactly why this is the case, but it very much should be.} $\dot u \eta a = s$.
  ...
  \todo{some map, haven't worked this out yet either.} ...
